...
void GSMtest(void) {
    ...
    CTOS_LCDTPrintXY(1, 2, "1. SIM1         ");
    CTOS_LCDTPrintXY(1, 3, "2. SIM2         ");
    CTOS_KBDGet(&key);

    CTOS_LCDTPrintXY(1, 2, "GSM Open:       ");
    CTOS_LCDTPrintXY(1, 3, "                ");
    // Open GSM channel and initial to start to use the GSM functions
    rc = CTOS_GSMOpen(9600, 1);
    if (rc != d_OK) {
        sprintf(sBuf, "%X ", rc);
        CTOS_LCDTPrintXY(10, 2, sBuf);
        CTOS_KBDGet(&key);
        return;
    }

    if (key == d_KBD_2) {
        CTOS_LCDTPrintXY(10, 2, "SIM2");
    ...
    CTOS_LCDTPrintXY(14, 2, "_ok     ");
    CTOS_Delay(1500);

    CTOS_GSMGetBAND(&bID);
    switch (bID) {
        case d_GSM_900_1800:
            CTOS_LCDTPrintXY(1, 2, "GSM_900_1800    ");
            break;
        case d_GSM_900_1900:
            CTOS_LCDTPrintXY(1, 2, "GSM_900_1900    ");
            break;
        case d_GSM_850_1800:
            CTOS_LCDTPrintXY(1, 2, "GSM_850_1800    ");
            break;
        case d_GSM_850_1900:
            CTOS_LCDTPrintXY(1, 2, "GSM_850_1900    ");
            break;
        default:
            CTOS_LCDTPrintXY(1, 2, "GSMGetBAND error");
            break;
    }
    ...
    do {
        ...
        CTOS_LCDTPrintXY(1, 3, "ChkRdy:    ");
        rc = CTOS_SIMCheckReady();
        sprintf(str, "%X", rc);
        CTOS_LCDTPrintXY(8, 3, str);
    ...
    if (rc == d_GSM_SIM_READY) {
        ...
        CTOS_LCDTPrintXY(8, 3, "fail ");
        return;
    }

    // Get the GPRS network registration status //
    memset(sBuf, 0x00, sizeof (sBuf));
    CTOS_LCDTPrintXY(1, 3, "RegState:       ");
    start = CTOS_TickGet();
    do {
...
void GPRSOpen(void) {
    // Initialize the GPRS //
    CTOS_LCDTPrintXY(1, 1, "[GPRSInit]");
    CTOS_TCP_GPRSInit();

    // GPRSOpen //
    CTOS_LCDTPrintXY(1, 2, "OPENING...         ");
    ...
    if (state != TRUE) {
        CTOS_LCDTPrintXY(1, 7, "Open fail    ");
        CTOS_KBDGet(&key);
        return;
    }
...
void GPRSConnect(void) {
    // GPRSConnect //
    CTOS_LCDTPrintXY(1, 2, "Connecting...      ");
    sprintf(str, "%d.%d.%d.%d\n%d", baIP_R[0], baIP_R[1], baIP_R[2], baIP_R[3], usPort);
    CTOS_LCDTPrintXY(1, 5, str);
    ret = CTOS_TCP_GPRSConnectEx(&bSocket, baIP_R, usPort);
    state = Check_state(ret);
    if (state != TRUE) {
        CTOS_LCDTPrintXY(1, 7, "Connect fail     ");
        CTOS_KBDGet(&key);
        return;
    }
    CTOS_LCDTPrintXY(1, 2, "Connect  [OK]       ");
    CTOS_KBDGet(&key);
    return;
}

void GPRSTxRx(void) {
    CTOS_LCDTPrintXY(1, 2, "Sending...    ");
    ...
    state = Check_state(ret);
    if (state != TRUE) {
        CTOS_LCDTPrintXY(1, 6, "Tx fail     ");
        CTOS_KBDGet(&key);
        return;
    }

    TotalLen = 0;
    CTOS_LCDTPrintXY(1, 2, "Receiving...    ");
    do {
...
void GPRSDisconnect(void) {
    CTOS_LCDTPrintXY(1, 2, "Disconnect...    ");
    ret = CTOS_TCP_GPRSDisconnect(bSocket);
    state = Check_state(ret);
    if (state != TRUE) {
        CTOS_LCDTPrintXY(1, 8, "Disconnect fail     ");
        CTOS_KBDGet(&key);
        return;
    }
}
...
void GPRSClose(void) {
    CTOS_LCDTPrintXY(1, 2, "CLOSING...    ");
    ret = CTOS_TCP_GPRSClose();
    state = Check_state(ret);
    if (state != TRUE) {
        CTOS_LCDTPrintXY(1, 8, "Close fail     ");
        CTOS_KBDGet(&key);
        return;
    }
   CTOS_KBDGet(&key); 
...
void gsmmain(void) {
    ...
    while (1) {
        ...
        CTOS_LCDTPrintXY(3, 1, "   GPRS GSM Set");
        CTOS_LCDTPrintXY(3, 3, "1. Select SIM");
        CTOS_LCDTPrintXY(3, 4, "2. Internet ON");
        CTOS_LCDTPrintXY(3, 5, "3. Test Internet");
        CTOS_LCDTPrintXY(3, 6, "4. Test Conn");
        CTOS_LCDTPrintXY(3, 7, "5. Disconnect");
        CTOS_LCDTPrintXY(3, 9, "6. Internet OFF");
        CTOS_LCDTPrintXY(3, 13, "X. Exit");
        CTOS_KBDGet(&key);
        CTOS_LCDTClearDisplay();
